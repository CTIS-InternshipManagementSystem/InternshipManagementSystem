...
import 'web_download_helper.dart'
    if (dart.library.io) 'mobile_download_helper.dart';
...
class _SearchPageState extends State<SearchPage> {
  ...
  void initState() {
    super.initState();
    _selectedOption = 'Search Student';
  ...
  void _searchSubmissions(String query) {
    setState(() {
      if (_selectedOption == 'Search by Supervisor' &&
          _selectedSupervisorId != null) {
        _futureStudentCourses = _getStudentsBySupervisor(
        ...
        _futureStudentCourses = DBHelper.getStudentCoursesWithCourseInfo().then(
          (submissions) =>
              submissions
                  .where(
                    (submission) =>
                        submission['bilkentId'].toString().startsWith(query),
                  )
                  .toList(),
        );
      }
  ...
  Future<List<Map<String, dynamic>>> _getStudentsBySupervisor(
    String supervisorId,
  ) async {
    try {
      final allStudentCourses =
          await DBHelper.getStudentCoursesWithCourseInfo();
      final studentsWithSupervisor = await DBHelper.getStudentsBySupervisor(
        supervisorId,
      );
      final bilkentIds =
          studentsWithSupervisor.map((s) => s['bilkentId'].toString()).toSet();

      return allStudentCourses
          .where((submission) => bilkentIds.contains(submission['bilkentId']))
          .toList();
    } catch (e) {
  ...
  void _showCenteredSuccessNotification(
    String operation,
    String fileName, {
    int? fileSize,
  }) {
          ...
          child: Center(
            child: Stack(
              ...
              children: [
                ...
                Icon(icon, color: color, size: 50 * value),
              ],
            ),
          ),
        );
  ...
  Future<void> uploadFile(
    ...
    try {
      ...
      final String destinationBase =
          "$year $semester/CTIS$code/${name}_$bilkentId";
      ...
      if (kIsWeb) {
        ...
        await storageRef
            .putData(_uploadFileBytes[bilkentId]!)
            .whenComplete(() => null);
      } else {
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            _buildRadioOption('Search Student', 'Search Student', textColor),
            _buildRadioOption(
        ...
        builder: (context, snapshot) {
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              final companyEvaluationUploaded =
                  submission['companyEvaluationUploaded'] ?? false;
                      ...
                      if (_selectedOption ==
                        ...
                        if (companyEvaluationUploaded) ...[
                          ElevatedButton(
                            ...
                            child: const Text(
                              'Download Company Evaluation Report',
                            ),
                          ),
                          const SizedBox(height: 8),
                          ElevatedButton(
                            ...
                            child: const Text(
                              'Delete Company Evaluation Report',
                            ),
                          ),
                        ] else ...[
                          ElevatedButton(
                            onPressed: () async {
                              if (kIsWeb) {
                                ...
                                if (result != null) {
                                  setState(() {
                                    _uploadFileBytes[bilkentId] =
                                        result.files.single.bytes;
                                    _uploadFilePaths[bilkentId] =
                          ...
                          if (_uploadFilePaths[bilkentId] != null)
                            Padding(
                              ...
                              child: Text(
                                'Selected file: ${_uploadFilePaths[bilkentId]}',
                              ),
                            ),
                          const SizedBox(height: 8),
                          ElevatedButton(
                        ...
                        ElevatedButton(
                          onPressed: () {
                            ...
                            Navigator.push(
                              ...
                              MaterialPageRoute(
                                builder:
                                    (context) => EvaluatePage(
                                      submission: submissionMap.map(
